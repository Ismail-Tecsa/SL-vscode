codeunit 50000 "Sunlight Event Handling"
{
  ...
  var FlowFieldsEditable: Boolean)var FieldRef: FieldRef;
  RecNo: Code[20];
  begin
    case RecRef.Number of DATABASE::Taxis, DATABASE::permit: begin
  ...
  var TotalAddCurrAmount: Decimal)begin
  ...
  local procedure "Lock-OpenServContract_OnBeforeLockServContract"(var ServiceContractHeader: Record "Service Contract Header")var BookEntry: record "Booking Entry";
  ...
  begin
    ...
    if not BookEntry.findlast then nextentryno:=0
    else
      nextentryno:=BookEntry."Entry No.";
    repeat nextentryno+=1;
      clear(BookEntry);
  ...
  local procedure "Lock-OpenServContract_OnAfterOpenServContract"(var ServiceContractHeader: Record "Service Contract Header")var BookEntry: record "Booking Entry";
  begin
    bookentry.reset;
